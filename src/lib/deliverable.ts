...
function buildMarkdown(job: WorkflowJob) {
  ...
  const lines = [
    ...
    `- Publish status: ${job.brief.publishStatus}`,
    `- Category IDs: ${job.brief.categoryIds.join(", ") || "-"}`,
    `- Tag IDs: ${job.brief.tagIds.join(", ") || "-"}`,
    `- Featured image URL: ${job.brief.featuredImageUrl || "-"}`,
    "",
    "## Brief",
    "",
    `${job.brief.angle}`,
